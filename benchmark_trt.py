...
def compute_gt_metrics(pred: np.ndarray, gt: np.ndarray) -> dict:
    ...
    Uses median scaling to align prediction to GT scale.
    Valid mask: gt > 0.

    Returns abs_rel, sq_rel, rmse, d1, d2, d3.
    """
    valid = gt > 1e-8
        ...
        if not metric_image_paths:
            ...
            for img_path in metric_image_paths:
                ...
                img_gt = cv2.resize(img_gt, (iw, ih),
                                    interpolation=cv2.INTER_CUBIC)
                img_gt = 1.0/img_gt # convert to disparity
                ...
                for ed, res in zip(engine_data, engine_results):
                    ...
                    depth_hw = squeeze_depth_to_hw(out_arr, out_name)
                    depth_resized = cv2.resize(depth_hw, (iw, ih),
                                               interpolation=cv2.INTER_CUBIC)
